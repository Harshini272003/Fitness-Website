...
<main class="form-signin w-50 m-auto">
  <form action="<?=$_SERVER['PHP_SELF']?>" method="post" >
    ...
    <h1 class="h3 mb-3 fw-normal text-white">Edit User Details</h1>
...
<?php
  $option = $_POST['option'];
  $q="select * from user where name='$option'";
...
?>
 ...
 <div class="form-floating">
        <button class="w-100 btn btn-md btn-primary" type="submit">Edit User Details</button>
  </div>
    </div>
</form  >
</main>
...
<form   action="<?=$_SERVER['PHP_SELF']?>" method="post">
     ...
     <div class="form-floating">
      <input type="text" class="form-control" id="name" placeholder="User Name" name="name" value='<?php echo $name;?>' required>
...
if (!empty($_POST['name']))
{
  ...
  $query = mysqli_query($con, "SELECT * FROM user WHERE id='$id' && name='$name' &&  email='$email' && password= '$password' && usertype='$usertype' ");
  $n = mysqli_num_rows($query); 
  $ro=mysqli_fetch_assoc($query);

if($n>0)
  {
      $message = "User aldready there";
  ...
  {
    $insert = "update user set name='$name',email='$email',password='$password',usertype='$usertype' where email='$email'";
