...
<main class="form-signin w-50 m-auto">
  <form action="<?=$_SERVER['PHP_SELF']?>" method="post" >
    ...
    <h1 class="h3 mb-3 fw-normal text-white">Delete a user</h1>
...
$num=mysqli_num_rows($result);
 ...
 ?>
    ...
    <button class="w-100 btn btn-md btn-primary" type="submit" id="delete">Delete User</button>
 </form><br>
</main>
</div>
...
if (!empty($_POST['option']))
{
  ...
  $query = mysqli_query($con, "SELECT * FROM user WHERE name='$option'");
  $n = mysqli_num_rows($query); 
  $ro=mysqli_fetch_assoc($query);

if($n>0)
  {
      $delete = "delete from user where name='$option'";
...
<script type="text/javascript">
  $(document).ready(function()
  {
      $("#delete").click(function()
      {
         ...
         if(name=="User Name")
         {
          alert("Select user name to delete");
         }
